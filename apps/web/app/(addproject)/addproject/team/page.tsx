...
//         if (selectedFile) {
//             try {
//                 const { data, error } = await supabase.storage.from('teamMembersImage').upload(`team-member-${index}`, selectedFile);
//                 if (error) {
//                     throw error;
//                 }
//                 if (data) {
//                     const supabaseUrl = process.env.NEXT_PUBLIC_SUPABASE_URL;
//                     const imageUrl = `${supabaseUrl}/storage/v1/object/public/teamMembersImage/team-member-${index}`;
...
//                     </a>
//                     <button type="submit">
//                     <a  className="flex gap-5 font-nunito justify-between items-center sm:-mr-12 px-4 py-2 text-white text-opacity-80 bg-[#954AD2] rounded-[15px]">
//                         <div>Next</div>
//                         <Image
//                             alt=""
//                             width={100}
  ...
  const handleSubmit = async (e: React.FormEvent): Promise<void> => {
    ...
    try {
        ...
        const teamData = members.map((member) => ({
            ...
            github: member.github,
            twitter: member.twitter,
            image: member.image instanceof File ? null : member.image, // Fix: Ensure image is of type string | null
        }));
