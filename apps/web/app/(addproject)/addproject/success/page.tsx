...
const Title: React.FC<TitleProps> = ({ text, className }) => {
    return <h1 className={`text-6xl tracking-wider leading-8 text-purple-500 capitalize max-md:max-w-full max-md:text-4xl ${className}`}>{text}</h1>;
};
...
const Subtitle: React.FC<SubtitleProps> = ({ text, className }) => {
    return <h2 className={`self-stretch mt-24 text-5xl tracking-wider leading-8 text-purple-300 capitalize max-md:mt-10 max-md:max-w-full max-md:text-4xl ${className}`}>{text}</h2>;
};
