...
const UserOnBoarding = () => {
    ...
    return (
        <form onSubmit={handleSubmit} className="flex flex-col self- sm:mt-24 font-nunito max-w-full mt-10">
            <div className="flex gap-2.5 self-start">
                {[1, 2, 3].map(key => (
                    <div key={key} className={`shrink-0 rounded-2xl bg-${key === 2 ? '[#954AD2]' : 'zinc-400'} h-[17px] w-[38px]`} />
                ))}
            </div>
            ...
            {['Name', 'Bio'].map((label, index) => (
                <div key={index} className="flex flex-col gap-5 justify-between w-full text-2xl tracking-wide leading-8 whitespace-nowrap sm:flex-none flex-wrap mt-10 max-w-full">
